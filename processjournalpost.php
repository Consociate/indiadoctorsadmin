				...
				if ((($_FILES["file"]["type"] == "application/msword")
					...
					|| ($_FILES["file"]["type"] == "application/octet-stream")))
					  ...
					  if ($_FILES["file"]["error"] > 0)
					    ...
					    switch($type){
									case 1:
											header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
										break;
									case 2:
											header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
										break;
					    ...
					    {
						$pathname = LIBRARY_JOURNAL_UPLOAD_FOLDER_PATH;
...
//						      	 echo "Stored in: " . $pathname . $finalfilename."</h1>";
						      ...
						      switch($type){
									case 1:
										if(addJournalNationalPost($userid, $post, $status, $dateofaddition, $sid, $views, $postlabel, $finalfilename)){
											header('Location: journalposts.php?sid='.$sid.'&eid=1&type='.$type);
										}else{
											header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
										}
										break;
									case 2:
										if(addJournalInterNationalPost($userid, $post, $status, $dateofaddition, $sid, $views, $postlabel, $finalfilename)){
											header('Location: journalposts.php?sid='.$sid.'&eid=1&type='.$type);
										}else{
											header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
										}
...
//						      	echo "<h1>File not uploaded. Please try again</h1>";
					...
					switch($type){
						case 1:
							if(addJournalNationalPost($userid, $post, $status, $dateofaddition, $sid, $views, $postlabel, null)){
								header('Location: journalposts.php?sid='.$sid.'&eid=1&type='.$type);
							}else{
								header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
							}
							break;
						case 2:
							if(addJournalInterNationalPost($userid, $post, $status, $dateofaddition, $sid, $views, $postlabel, null)){
								header('Location: journalposts.php?sid='.$sid.'&eid=1&type='.$type);
							}else{
								header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
							}
...
$postid = $_GET['tid'];
		...
		switch($type){
			case 1:
		if(approveJournalNationalPosts($postid, $status, $_SESSION['uid'])){
			header('Location: journalposts.php?sid='.$sid.'&eid=1&type='.$type);
		}else{
			header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
		}
				break;
			case 2:
		if(approveJournalInterNationalPosts($postid, $status, $_SESSION['uid'])){
			header('Location: journalposts.php?sid='.$sid.'&eid=1&type='.$type);
		}else{
			header('Location: journalposts.php?sid='.$sid.'&eid=2&type='.$type);
		}
