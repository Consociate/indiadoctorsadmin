        ...
        <div id="content" class="clearfix">
            <div class="contentwrapper"><!--Content wrapper-->
                ...
                include_once 'src/controllers/surveycontroller.php';
                	$postdetails = showSuvreyPosts($tid);
                	$post = $postdetails[0];
                ?>
                        <li class="active"><?php echo substr($post->getPostlabel(), 0, 29)?></li>
                    </ul>
                ...
                <div class="span12">
					<?php 
                        	if($_GET['eid']==1){
                                ...
                                <button type="button" class="close" data-dismiss="alert">&times;</button>
                                <strong>Oh snap!</strong> Error updating your vote. Please try again later.
                            </div>
                        		<?php 
                        	}
                        ?>
				...
				<div class="span12">
							...
							<?php 
								$totalnoofvotes = showTotalNoOfVotesForSurvey($tid);
							?>
                            <div class="title">

                                <h4>
                                    <span class="icon16 icomoon-icon-pie"></span>
                                    <span><?php echo $post->getPostlabel()?> <span class="label label-success"><span class="icomoon-icon-arrow-up white"></span><?php echo $totalnoofvotes?></span></span>
                                </h4>
                            ...
                            <div class="content">
                                ...
                                <div class="vital-stats" style="padding-bottom:23px">
                                    <ul class="unstyled">
                                    <?php 
                                    	$surveyoptions = showSurveyOptions($tid);
                                    	$showvotingoptions = checkIfUserhasVotedForThisSurvey($_SESSION['uid'], $tid);
                                    	foreach($surveyoptions as $option){
                                    		$percent = 0;
                                    		$optionvotes = showNoofVotesForSurveyOption($tid, $option->getOptionid());
                                    		if($totalnoofvotes){
                                    		...
                                    		if(!$showvotingoptions){
                                    			echo '<a href="processsurveypost.php?action=2&tid='.$tid.'&oid='.$option->getOptionid().'" class="pull left btn btn-info marginR10">Vote</a>';
                                    		}
                                            echo '
