            ...
            <div class="contentwrapper"><!--Content wrapper-->
                ...
                <div class="row-fluid">
                        ...
                        <div class="centerContent">
                            ...
                            <ul class="bigBtnIcon">
                                <li>
                                    <a href="addadverts.php" title="Add Advertisements" class="tipB">
                                        <span class="icon brocco-icon-plus"></span>
                                        <span class="txt">Add Advertisements</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
					...
					<?php 
                        	if($_GET['eid']==1){
                                ...
                                <strong>Advertisements Updated!</strong> 
                            </div>
                        		<?php 
                        	}else if($_GET['eid']==2){
                        		?>
                                ...
                                <button type="button" class="close" data-dismiss="alert">&times;</button>
                                <strong>Oh snap!</strong> Error updating Advertisements. Please try again later.
                            </div>
                        		<?php 
                        	}
                        ?>
					...
					<div class="span12">
                            ...
                            $postlist = showAdvertsPosts();
                            $index = 0;
                            foreach($postlist as $post){
                            	$slot = '';
                            	switch($post->getPost()){
                            		case AD_SLOT_BELOW_EDITORS_POST:
                            			$slot='Below Editor\'s Post';
                            			break;
                            		case AD_SLOT_BELOW_USER_ACTIVITY:
                            			$slot='Below User Activity';
                            			break;
                            		case AD_SLOT_BELOW_SIDE_SCROLL:
                            			$slot = 'Below Side Scroll';
                            			break;
                            	}
                            	echo '<tr>
                                    <td style="text-align:left;">'.($index+1).'</td>
                                    <td>'.$slot.'</td>
                                    <td>'.$post->getPostlabel().'</td>
                                    <td>'.date('d-m-Y', strtotime($post->getDateofaddition())).'</td>
                                    <td>'.$post->getCreatedby().'</td>';
                            if($role == USER_ADMIN){
                            			echo '<td><a href="processadverts.php?action=3&tid='.$post->getTid().'&st=0" title="Remove This" class="tip">
