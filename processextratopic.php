...
//					    	echo "Return Code: " . $_FILES["file"]["error"] . "<br />";
							header('Location: extratopics.php?eid=2');
					    }
					  else
					    {
...
//					    echo "Temp file: " . $_FILES["file"]["tmp_name"] . "<br />";
						$pathname = EXTRA_UPLOAD_FOLDER_PATH;
...
//						      	 echo "Stored in: " . $pathname . $finalfilename."</h1>";
						      ...
						      if(addExtratopicPost($userid, $post, $status, $views, $dateofaddition, $postlabel, $finalfilename)){
			header('Location: extratopics.php?eid=1');
		}else{
			header('Location: extratopics.php?eid=2');
		}
						     }else{
//						      	echo "<h1>File not uploaded. Please try again</h1>";
			...
			header('Location: extratopics.php?eid=2');
				...
				if(addExtratopicPost($userid, $post, $status, $views, $dateofaddition, $postlabel, null)){
    		header('Location: extratopics.php?eid=1');
		}else{
			header('Location: extratopics.php?eid=2');
		}
...
//add post comment
	...
	case 3:
		...
		$userid = $_SESSION['uid'];
		$status = $_GET['st'];
		approveExtratopicPosts($tid, $status, $userid);
		header('Location: extratopics.php');
		break;
}
